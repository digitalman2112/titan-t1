---
permalink: /accessories/shoulder-pads/
title: "Shoulder Pads"
code: "S5"
owned: true
description: "A left/right pair of padded shoulder supports (S5-L and S5-R) that attach to the smith carriage crossbar, allowing the carriage to rest on the shoulders for squat and lunge variations."
photos:
  - /assets/images/accessories/s5-1.png
  - /assets/images/accessories/s5-2.png
  - /assets/images/accessories/1-Shoulder-Pads.png
videos:
  # - youtube_id: ""
  #   title: "Shoulder Pads â€” Setup"
---

Shoulder pads insert into the smith carriage crossbar. Insert S5-L into hole **16-L** and S5-R into hole **16-R**. Ensure both pads are fully seated and will not slide out before loading weight or getting under the carriage.

**Storage:** Remove shoulder pads and store on the machine's storage rack when not in use.
